...
# =========================================================
def business_override(question):
    q = question.lower()

    # ACTIVE CUSTOMER (supports multiple variations)
    if any(keyword in q for keyword in ["active customer", "most active", "top customer", "active users"]):
        return """
MATCH (c:Customer)
OPTIONAL MATCH (c)-[:PLACED]->(o:SalesOrder)
WITH c, count(DISTINCT o) AS orders
OPTIONAL MATCH (c)-[:MADE_PAYMENT]->(p:Payment)
WITH c, orders, count(DISTINCT p) AS payments
WITH c, (orders + payments) AS activity
WHERE activity > 0
RETURN c.id AS customerId, activity
        ...
        for value in record.values():
            ...
            if hasattr(value, "nodes"):
                for node in value.nodes:
                    ...
                    if nid not in seen:
                        nodes.append({"id": nid, "type": label})
                        seen.add(nid)
...
def visualize_graph(graph_data):
    ...
    for node in graph_data["nodes"]:
        if node["id"] in graph_data["highlight"]:
            net.add_node(node["id"], label=node["id"], color="red", size=25)
        else:
...
def is_valid_query(user_input):
    allowed = ["order", "delivery", "invoice", "payment", "customer", "product"]
...
if st.button("Ask"):
    ...
    if not user_input:
        ...
        st.error("This system is designed for dataset-related queries only.")

    else:
        try:
            # PRIORITY PIPELINE
            cypher_query = business_override(user_input)
